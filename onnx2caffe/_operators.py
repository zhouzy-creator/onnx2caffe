...
opset_version = 11 # 设置一个全局变量, 此时为默认的 opset_version
...
def _convert_Reshape(node,graph,err):
    ...
    print("reshape input",node.input_tensors[node.inputs[1]])
    ...
    if len(shape) == 2:
        ...
        return layer
    if len(shape) == 3:
        print("reshape len", len(shape), shape)
        shape_l = list(shape)
        layer = myf("Flatten", node_name, [input_name], [output_name], axis=2)
        graph.channel_dims[output_name] = shape[1]
        return layer
    elif len(shape) == 4:
        print("reshape4 len", len(shape), shape)
        shape_l = list(shape)
        # hard code, reshpae only C/H/W, not N
        print("xxshape_1:",shape_l)
        shape_l[0] = 1

        #if(shape_l[1]==-1):
         #   shape_l[1]=116
        print("shape_1:", shape_l)
        graph.channel_dims[output_name] = shape[1]
        print( dict(shape=dict(dim=shape_l)))
        layer = myf("Reshape", node_name, [input_name], [output_name], reshape_param = dict(shape=dict(dim=shape_l)))
        return layer
    elif len(shape) == 5:
        print("reshape len", len(shape), shape)
        shape_l = list(shape)
        graph.channel_dims[output_name] = shape[2]
        layer = myf("Reshape", node_name, [input_name], [output_name], reshape_param = dict(shape=dict(dim=shape_l)))
        return layer
    else:
        print("reshape len",len(shape),shape)
...
def _convert_upsample2(node,graph,err):
    ...
    print("scales",node_name,node.attrs,mode ,len(node.inputs))
    print("scales inputs",node.inputs[0],node.inputs[1],node.inputs[2],node.inputs[3])
    if(mode==b'pytorch_half_pixel'):
        height_scale = 2
        width_scale = 2
...
def _convert_conv_slice(node, graph, err):
    ...
    #channels = graph.channel_dims[input_name]
    channels = graph.shape_dict[input_name][1]
    #print(" slic",axes,len(axes),channels,output_name)
    #if len(axes) != 1:
    ...
    output_name_list = [output_name, str(output_name) + "slice_another"]
    layer = myf('Slice', node_name, [input_name], output_name_list, slice_dim=1, slice_point=int(channels/2))
    graph.channel_dims[output_name_list[0]] = int(channels/2)
    graph.channel_dims[output_name_list[-1]] =int( channels/2)
    return layer
def _convert_split(node, graph, err):
    print("split", len(node.inputs),len(node.outputs))
    #print(node.attrs)
    input_name = str(node.inputs[0])
    output_name1 = str(node.outputs[0])
    output_name2 = str(node.outputs[1])
    #print("split",output_name1,output_name2)
    output_name_list = [output_name1, output_name2]
    node_name = node.name
    axes = node.attrs.get('axes', [])
    ...
    graph.channel_dims[output_name_list[0]] = split[0]
    graph.channel_dims[output_name_list[1]] = split[1]
    #print(graph.channel_dims)
    layer = myf('Split', node_name, [input_name], output_name_list)
    return layer
_ONNX_NODE_REGISTRY = {
    "Conv": _convert_conv,
    "Relu": _convert_relu,
